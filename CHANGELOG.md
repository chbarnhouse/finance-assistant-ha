...
- **Complete Device Association**: Added missing device_info to TotalIncome, TotalExpenses, and NetCashFlow sensors
- **100% Entity Organization**: All 34 entities now properly belong to the Finance Assistant device
- **Mission Accomplished**: No more orphaned entities!

## [1.1.24] - 2025-08-16

### Fixed

- **Final Device Association**: Added missing device_info to first ObligationRatioSensor
- **Near Complete Entity Organization**: Now 31 out of 34 entities are properly associated with the device
- **Almost There**: Only 3 entities remaining to be fully organized

## [1.1.23] - 2025-08-16

### Fixed

- **Additional Device Association**: Added missing device_info to OverallFinancialScoreSensor, RiskLevelSensor, and second ObligationRatioSensor
- **Progress on Entity Organization**: Now 19 out of 34 entities are properly associated with the device
- **Continued Fix for Orphaned Entities**: Addressing remaining sensors that need device_info
...
- **Complete Device Association**: Added missing device_info attributes to ALL remaining sensors
- **Entity Organization**: All sensors now properly belong to the Finance Assistant device
- **No More Orphaned Entities**: Every sensor will appear organized under the Finance Assistant device

## [1.1.20] - 2025-08-16

### Fixed

- **USD Unit of Measure**: Added proper USD unit of measure to all monetary sensors
- **Currency Formatting**: All monetary sensors now display with proper $X.XX format like Total Assets and Total Liabilities
- **Consistent Display**: Cash Flow Forecast, Monthly Obligations, Upcoming Expenses, and all balance sensors now show as currency
...
- **Device Association Issue**: Fixed enhanced analytics sensors not being associated with Finance Assistant device
- **Entity Organization**: All sensors now properly belong to the Finance Assistant device in Home Assistant
- **Device Structure**: Sensors will now appear organized under the Finance Assistant device instead of as orphaned entities
...
- **Dashboard Data Fetching**: Fixed coordinator to actually call get_dashboard() method instead of returning empty data
- **Sensor Data**: Home Assistant sensors should now display real financial data instead of "Unknown"
- **Data Flow**: Resolved the disconnect between API client and coordinator data fetching
